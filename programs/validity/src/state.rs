...
pub fn reconstruct_public_inputs(prefix: &[u8], suffix: &[u8]) -> Vec<u8> {
    let mut public_inputs = Vec::with_capacity(prefix.len() + suffix.len());
    public_inputs.extend_from_slice(prefix);
