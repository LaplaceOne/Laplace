...
export const criteria: CriterionEntry[] = [
  { key: 'hashlock', name: 'Hashlock', kind: 'Preimage',
    ...
    commitment: 'criterion_data_hash = H(domain, program, version, intent_id, parties, asset, amount, expiry, SHA256(secret))',
    verify: 'H(intent fields, SHA256(fulfillment_data)) == criterion_data_hash',
    conformance: { binds: 'full', accounts: 0, interfaceVersion: 2,
      note: 'Binds the intent fields (id, parties, asset, amount, expiry) into the commitment, so a revealed secret cannot be replayed against another intent. The shared secret still unlocks every leg of an atomic swap.' },
