...
The current `programs/hashlock` adapter treats `criterion_data_hash` as the SHA-256 hashlock itself and accepts a fulfillment when `SHA256(fulfillment_data) == criterion_data_hash`.
