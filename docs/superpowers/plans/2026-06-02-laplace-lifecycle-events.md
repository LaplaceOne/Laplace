...
`target/` is gitignored, so `target/idl/*.json` are **untracked build artifacts** and must NOT be committed —
they are reproducibly regenerated from committed source by `anchor build`. `app/packages/sdk/src/generated`
is tracked but Codama produces no event diff. So this step usually commits nothing; only commit if
`generated/` actually changed:
```bash
git add app/packages/sdk/src/generated
git diff --cached --quiet || git commit -m "chore(sdk): regenerate Codama client"
```
...
## As-built notes (deviations from the literal plan)

Tasks 1–5 were executed and independently verified. Functionally-equivalent deviations made during implementation:

1. **Event discriminator helper (Rust tests):** the plan's `anchor_lang::solana_program::hash::hash` does not exist in anchor-lang 1.0.2. Replaced with `solana_sha256_hasher::hash` (the same SHA-256 hasher the programs use). The `solana-sha256-hasher` dev-dep was added to `programs/hashlock/Cargo.toml` (validity already had it). Discriminator semantics `sha256("event:<Name>")[..8]` are unchanged; the RED→GREEN e2e proves the computed discriminators match the real on-chain emitted bytes.
2. **IDL is not committed (Task 3):** `target/` is gitignored, so `target/idl/*.json` are untracked build artifacts (regenerated by `anchor build`). Codama produced no diff for `app/packages/sdk/src/generated`. Task 3 therefore produced no commit — correct given repo hygiene; the events live in the committed *source*, not the IDL.
3. **`parseLaplaceEvents` regex guard (Task 4):** under this tsconfig's `noUncheckedIndexedAccess`, `m[1]` is `string | undefined`; guarded with `if (!m?.[1]) continue;` before base64-decoding. Behavior identical.
4. **`validity/src/lib.rs` module order:** `events` placed alphabetically (after `error`, before `state`) to match the file's existing ordering. Same compiled result.

**Commits on `feat/lifecycle-events`:** `8418cd4` (laplace events) · `d871f0d` (validity event) · `16c9251` (SDK decoder) · `9688e23` (SDK integration test).

**Remaining:** Task 6 (devnet redeploy) — on-chain upgrade-in-place, run when ready.
